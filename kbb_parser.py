...
for car in cars:
    ...
    chosen_mpg = car.findChildren("div", {"class": "css-ba3sq5"})[0].findChildren("div")[0].text
    chosen_mpg = chosen_mpg.split(" ")[0]
    chosen_horsepower = 0
    if chosen_car not in relations.keys():
        relations[chosen_car] = [float(chosen_rating.strip()), chosen_mpg]
print(relations)
